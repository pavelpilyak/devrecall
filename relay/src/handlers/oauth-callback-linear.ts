...
export async function handleLinearOAuthCallback(
  ...
  const redirectUri = `${url.origin}/oauth/linear/callback`;

  // Exchange authorization code for access token.
  const tokenResp = await fetch(LINEAR_TOKEN_URL, {
    method: "POST",
  ...
  const tokenData: LinearTokenResponse = await tokenResp.json();

  if (tokenData.error || !tokenData.access_token) {
    return htmlResponse(
      "Authorization Failed",
      `Linear returned an error: ${tokenData.error_description || tokenData.error || "unknown error"}`,
      400
    );
  }

  // Fetch user info via GraphQL viewer query.
  const viewerResp = await fetch(LINEAR_GRAPHQL_URL, {
    method: "POST",
    headers: {
  ...
  if (viewerResp.ok) {
    ...
    } = await viewerResp.json();
    userId = viewerData.data?.viewer?.id || "";
    userName = viewerData.data?.viewer?.name || "";
    email = viewerData.data?.viewer?.email || "";
  }

  const storedToken: StoredLinearToken = {
